/**
 * Hades Army — D1 Database Client
 * All D1 operations go through this client.
 */

import type { HadesEnv } from '../config/env';
import type {
  Project, Task, TaskStateTransition, Review, AgentRun,
  Approval, ModelUsage, SystemLog, FileLock
} from '../types';
...
export class D1Client {
  ...
  async createUser(telegramId: number, settingsJson: string = '{}'): Promise<string> {
    ...
    await this.db.prepare(
      `INSERT INTO users (id, telegram_id, settings_json, created_at)
       VALUES (?, ?, ?, ?)`
    ).bind(id, telegramId, settingsJson, new Date().toISOString()).run();
    return id;
  }
  ...
  async createProject(project: Omit<Project, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    ...
    await this.db.prepare(
      ...
      id, project.userId, project.name, project.repoUrl, project.repoName,
      project.repoOwner, project.defaultBranch, project.status,
      project.githubTokenEncrypted, now, now
    ).run();
    return id;
  }
  ...
  async createTask(task: Omit<Task, 'id' | 'createdAt' | 'updatedAt' | 'retryCount'>): Promise<string> {
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
    await this.db.prepare(
      ...
      id, task.projectId, task.title, task.description, task.state, task.priority,
      task.assignedAgent, task.parentTaskId ?? null,
      JSON.stringify(task.dependencies),
      JSON.stringify(task.requiredFiles),
      JSON.stringify(task.constraints),
      task.expectedOutput,
      now, now, 0, task.maxRetries ?? 3
    ).run();
    return id;
  }
  ...
  async updateTaskState(id: string, newState: Task['state'], reason: string): Promise<void> {
    ...
    ).bind(newState, now, id).run();

    // Log transition
    await this.db.prepare(
      `INSERT INTO task_states (id, task_id, previous_state, new_state, triggered_by, reason, timestamp)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(crypto.randomUUID(), id, task.state, newState, 'system', reason, now).run();
  }
  ...
  async createAgentRun(run: Omit<AgentRun, 'id'>): Promise<string> {
    ...
    await this.db.prepare(
      ...
      id, run.taskId, run.projectId, run.agentRole, run.model, run.provider,
      run.promptTokens, run.completionTokens, run.totalTokens, run.costEstimate,
      run.startTime, run.endTime ?? null, run.durationMs ?? null,
      run.status, run.output ?? null, run.error ?? null
    ).run();
    return id;
  }

  async updateAgentRun(id: string, updates: Partial<Pick<AgentRun, 'endTime' | 'durationMs' | 'status' | 'output' | 'error'>>): Promise<void> {
    const sets: string[] = [];
  ...
  async createFileLock(filePath: string, taskId: string, expiresAt: string): Promise<void> {
    ...
    ).bind(filePath, taskId, new Date().toISOString(), expiresAt).run();
  }
  ...
  async getFileLock(filePath: string): Promise<FileLock | null> {
    ...
    ).bind(filePath).first<Record<string, unknown>>();
    return result ? {
  ...
  async getLocksByTask(taskId: string): Promise<FileLock[]> {
    const { results } = await this.db.prepare(
      `SELECT * FROM file_locks WHERE task_id = ?`
  ...
  async createApproval(approval: Omit<Approval, 'id'>): Promise<string> {
    ...
    await this.db.prepare(
      ...
      id, approval.taskId, approval.prNumber, approval.status,
      approval.requestedAt, approval.respondedAt ?? null,
      approval.responderTelegramId ?? null, approval.comment ?? null
    ).run();
    return id;
  }
  ...
  async updateApprovalStatus(
    ...
    ).bind(status, new Date().toISOString(), responderTelegramId, comment ?? null, taskId).run();
  }
  ...
  async recordModelUsage(usage: Omit<ModelUsage, 'id'>): Promise<void> {
    await this.db.prepare(
      ...
      crypto.randomUUID(), usage.provider, usage.model, usage.tokensUsed,
      usage.cost, usage.success ? 1 : 0, usage.durationMs, usage.timestamp
    ).run();
  }
